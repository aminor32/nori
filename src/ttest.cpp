...
class StudentsTTest : public NoriObject {
   public:
    StudentsTTest(const PropertyList &propList) {
        ...
        /* This parameter specifies a list of incidence angles that will be
         * tested */
        std::vector<std::string> angles =
            tokenize(propList.getString("angles", ""));
        for (auto angle : angles) m_angles.push_back(toFloat(angle));

        /* This parameter specifies a list of reference values, one for each
         * angle */
    ...
    void activate() {
        ...
        if (!m_bsdfs.empty()) {
            if (m_references.size() * m_bsdfs.size() != m_angles.size())
                throw NoriException(
                    "Specified a different number of angles and reference "
                    "values!");
            if (!m_scenes.empty())
                throw NoriException(
                    "Cannot test BSDFs and scenes at the same time!");
            ...
            for (auto bsdf : m_bsdfs) {
                for (size_t i = 0; i < m_references.size(); ++i) {
                    ...
                    cout << "Drawing " << m_sampleCount << " samples .. "
                         << endl;
                    double mean = 0, variance = 0;
                    for (int k = 0; k < m_sampleCount; ++k) {
                        Point2f sample(random.nextFloat(), random.nextFloat());
                        double result =
                            (double)bsdf->sample(bRec, sample).getLuminance();

                        /* Numerically robust online variance estimation using
                           an algorithm proposed by Donald Knuth (TAOCP vol.2,
                           3rd ed., p.232) */
                    ...
                    std::pair<bool, std::string> result =
                        hypothesis::students_t_test(
                            mean, variance, reference, m_sampleCount,
                            m_significanceLevel, (int)m_references.size());
            ...
            if (m_references.size() != m_scenes.size())
                throw NoriException(
                    "Specified a different number of scenes and reference "
                    "values!");
            ...
            for (auto scene : m_scenes) {
                ...
                for (int k = 0; k < m_sampleCount; ++k) {
                    ...
                    /* Numerically robust online variance estimation using an
                       algorithm proposed by Donald Knuth (TAOCP vol.2, 3rd ed.,
                       p.232) */
